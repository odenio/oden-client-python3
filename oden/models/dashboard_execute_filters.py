...
class DashboardExecuteFilters(BaseModel):
    """
    ...
    """ # noqa: E501
    lines: Optional[List[Optional[DashboardExecuteFiltersLinesInner]]] = Field(default=None, description="Lines to restrict to. Each entry must supply `id`, `name`, or both; entries that supply neither are rejected. Other Line fields (factory, secondary_name, match) are not used here and are intentionally omitted so generated clients don't suggest them as inputs. ")
    shifts: Optional[List[StrictInt]] = None
    product_ids: Optional[List[UUID]] = None
    product_attribute_value_ids: Optional[List[UUID]] = None
    scrap_categories: Optional[List[UUID]] = None
    states: Optional[DashboardExecuteFiltersStates] = None
    custom_intervals: Optional[List[DashboardExecuteFiltersCustomIntervalsInner]] = None
    __properties: ClassVar[List[str]] = ["lines", "shifts", "product_ids", "product_attribute_value_ids", "scrap_categories", "states", "custom_intervals"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in custom_intervals (list)
        _items = []
        if self.custom_intervals:
            for _item_custom_intervals in self.custom_intervals:
                if _item_custom_intervals:
                    _items.append(_item_custom_intervals.to_dict())
            _dict['custom_intervals'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "lines": [DashboardExecuteFiltersLinesInner.from_dict(_item) for _item in obj["lines"]] if obj.get("lines") is not None else None,
            "shifts": obj.get("shifts"),
            "product_ids": obj.get("product_ids"),
            "product_attribute_value_ids": obj.get("product_attribute_value_ids"),
            "scrap_categories": obj.get("scrap_categories"),
            "states": DashboardExecuteFiltersStates.from_dict(obj["states"]) if obj.get("states") is not None else None,
            "custom_intervals": [DashboardExecuteFiltersCustomIntervalsInner.from_dict(_item) for _item in obj["custom_intervals"]] if obj.get("custom_intervals") is not None else None
        })
        return _obj
