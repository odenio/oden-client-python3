...
class DashboardColumnSpec(BaseModel):
    """
    DashboardColumnSpec
    """ # noqa: E501
    name: StrictStr
    ...
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['string', 'number', 'boolean', 'datetime']):
            raise ValueError("must be one of enum values ('string', 'number', 'boolean', 'datetime')")
        return value
